...
작업자 배치를 카톡 수동 → 자동으로. 관리자는 끝난 도크의 **"종료"만 누르면**, 휴게(기본 9분 50초)
...
```bash
npm install
npm start            # http://localhost:3000  (배정 6분 30초 / 휴게 9분 50초)
```
...
- 휴게 시작 **6분 30초** 뒤(`ASSIGN_DELAY_SEC`) 그 작업자에게 **가장 오래된 대기 도크**를 자동 배정.
- 실제 휴게 **9분 50초**(`BREAK_DELAY_SEC`) 동안 배정~복귀 사이는 보드에 "→ B29" 로 강조.
- 배정/휴게 시간은 관리 현황판 요약줄의 **"배정 …/휴게 …"를 눌러** 실시간 변경 가능(저장됨, 기본값보다 우선).
